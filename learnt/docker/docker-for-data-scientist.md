...
Data is not recommended to store inside container because once container is removed data will be removed together. Docker use `Volume` to mount directory on the host to container. See more details on [Docker Docs](https://docs.docker.com/storage/volumes/).
